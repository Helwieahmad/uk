...
<div id="koplak"><p align="center">Selamat Datang di Halaman Profil Dosen-Dosen Universitas Kanjuruhan Malang</p></div>
<tr>	
				<td>
...
<table width='617'>
							   ...
							   $w=mysql_fetch_array(mysql_query("SELECT * FROM prodi ORDER BY id_prodi"));
								$no = $posisi+1;
								while ($data = mysql_fetch_array($sql)){
									?>
									<tr height="30"> 
										<td align="center" hidden><?php echo $no; ?></td>
										<td  style="border-bottom:1px solid #6e6e6f;">&nbsp; <a style="color:#6e6e6f;font-family:Verdana, Geneva, sans-serif; font-size:14px;" href=""><?php echo $data[nama] ; ?></a></td>
                                        <td  style="border-bottom:1px solid #6e6e6f;">&nbsp; <a style=" color:#6e6e6f;font-family:Verdana, Geneva, sans-serif; font-size:14px;" href=""> <?php $tampil=mysql_query("SELECT * FROM prodi ORDER BY nama_prodi"); 
										while($w=mysql_fetch_array($tampil)){if ($data[nama_prodi]==$w[id_prodi]){echo $w[nama_prodi];}} ?>
                                     </a></td>
									</tr>
									<?php
									$no++;
